...
export class VaultService {
  ...
  async init(): Promise<void> {
    ...
    const userPasswordSalt = await this.crStore.get<string>("userPasswordSalt");
    if (userPasswordSalt) {
      ...
      await this.crStore.set<string>(
        "userPasswordSalt",
        Buffer.from(this.userPasswordSalt).toString("hex")
      );
    }

    const aesCounterSalt = await this.crStore.get<string>("aesCounterSalt");
    if (aesCounterSalt) {
    ...
    const vaultMap = await this.crStore.get<Record<string, Vault[]>>(
      "vaultMap"
    );
    if (vaultMap) {
      runInAction(() => {
    ...
    autorun(() => {
      ...
      this.crStore.set<Record<string, Vault[]>>(
        "vaultMap",
  ...
  async signUp(userPassword: string): Promise<void> {
    ...
    if (
      ...
      (await this.crStore.get("aesCounterCipher"))
    ) {
  ...
  async unlock(userPassword: string): Promise<void> {
    ...
    const aesCounterCipher = await this.crStore.get<string>("aesCounterCipher");
    if (!aesCounterCipher) {
  ...
  protected async getPasswordCryptoState(): Promise<
    ...
    | undefined
  > {
    const cipherText = await this.crStore.get<string>("passwordCipher");
    const macText = await this.crStore.get<string>("userPasswordMac");
  ...
  async clearAll(userPassword: string): Promise<void> {
    ...
    await Promise.all([
      this.crStore.set("passwordCipher", null),
      this.crStore.set("userPasswordMac", null),
      this.crStore.set("aesCounterCipher", null),
    ]);
